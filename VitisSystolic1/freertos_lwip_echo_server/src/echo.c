...
void process_echo_request(void *p)
{
	...
	Status1 = XGpio_Initialize(&Gpio_output, 0xa0000000);
...
int i;
    ...
    while (1) {
        ...
        //XGpio_DiscreteWrite(&Gpio_output, 1,k);
		//xil_printf("	%x	%x	%d	%d	",temp,temp << 1 | 1,k, k-4);
		//xil_printf("\n");
		//XGpio_DiscreteWrite(&Gpio_output, 1,0x00000000);
...
//=============================================================
		...
		if(counter==2){
			...
			Status2 = XGpio_Initialize(&Gpio_input, 0xa0000000);
			XGpio_SetDataDirection(&Gpio_input, 2, 0x0000FFFF);
			...
			int mask_appl;
			while(1){
			//	DataRead = XGpio_DiscreteRead(&Gpio_input, 2);
				//xil_printf("%d",DataRead);
				do{
					ktemp= DataRead;
					DataRead = XGpio_DiscreteRead(&Gpio_input, 2) ;
					//mask_appl = (DataRead & 0x0000001f6) >> 1;
					//xil_printf("...%d....%d...\n",DataRead,ktemp);
				}while(DataRead == ktemp);//mask_appl == ktemp) && 
				mask_appl = DataRead >> 1;
				if(mask_appl !=254 && mask_appl !=239){
				//ktemp = DataRead;
